...
| `loaderContext.get`      | `honoContext.get`             | 通过 `c.set` 注入数据后 dataLoader 中消费：旧版通过 `loaderContext.get` 获取，新版参考 [Plugin](/zh/guides/advanced-features/custom-server.html#使用姿势-2) 示例    |
