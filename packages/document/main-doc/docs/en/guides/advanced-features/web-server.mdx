...
| `loaderContext.get`      | `honoContext.get`             | After injecting data using `c.set`, consume in dataLoader: the old version uses `loaderContext.get`, refer to the new version in [Plugin](/zh/guides/advanced-features/custom-server.html#使用姿势-2) example    |
